...
<?  
	...
	if ($_GET['send']=="ok") {
							...
							$am=$a_name;
	                        $a1=$_POST['tb'];$a2=explode("&",$a1);$tb=$a2[0];
...
while($record=mysql_fetch_array($result)) {
	$n_id_tb=$record[id];
	$n_id_tb_name=$record[name];
}
?>
<Form action="<? echo $PHP_SELF."?send=ok&id=$id"; ?>" method="POST" name="save"  onSubmit="return check_cd()">
<center>
...
<table border="1" width="100%" bordercolorlight="#FFFFFF" bordercolordark="#FFFFFF" bgcolor="#805380" bordercolor="#000000" id="table2" height="70">
	<tr>
		<td>
		<p align="center"><font size="5" color="#00FF00"><b>แก้ไขข้อมูลของ <?echo $n_name." ".$n_surname;?></b></font>
		</td>
	</tr>
</table>
...
<?        $po="";
			...
			if ($po == ""){
                     echo "<option value=\"?po=$n_id_po&id=$id\">$n_po</option>"; 
                     echo "<option value=\"?po=$n_id_po&id=$id\">-----------------------------</option>"; 
			}else{ 
							$sql="SELECT * FROM province  WHERE id='$po' "; 
                            $result=mysql_db_query($_dbname,$sql);
                            while($record=mysql_fetch_array($result)) {
                    	              $p_id=$record[id];
                       	              $p_name=$record[name];
			                           echo "<option value=\"?po=$p_id&id=$id\" >$p_name</option><option value=\"?po=$p_id&id=$id\">----------------------------</option>";}
			                 }
            ...
            while($record=mysql_fetch_array($result)) {
                    	$p_id=$record[id];
                       	$p_name=$record[name];
                        echo "<option value=\"?po=$p_id&id=$id\">$show .) $p_name</option>";
						$show++;
			}
?>			
			</select>
...
<?
           ...
           if ($_GET['am'] != ""){$am=$_GET['am'];}
		    if ($am == ""){
                     echo "<option value=\"?am=$n_id_am&po=$n_po&id=$id\">$n_am</option>"; 
                     echo "<option value=\"?am=$n_id_am&po=$n_po&id=$id\">-----------------------------</option>"; 
			         }else{
						    $sql2="SELECT * FROM amphur  WHERE id='$am' "; 
                            $result2=mysql_db_query($_dbname,$sql2);
                            while($record=mysql_fetch_array($result2)) {
                    	              $a_id=$record[id];
                       	              $a_name=$record[name];
				                      echo "<option value=\"?am=$a_id&po=$po&id=$id\">$a_name</option>
									  <option value=\"?am=$a_id&po=$po&id=$id\">----------------------------</option>";}
					 }
            ...
            while($record=mysql_fetch_array($result2)) {
                    	$a_id=$record[id];
                       	$a_name=$record[name];
                        echo "<option value=\"?am=$a_id&po=$po&id=$id\">$show2 .) $a_name</option>";
						$show2++;
			}		
		?>
		</select>
...
<?        
	      if ($_POST['tb'] != ""){
                            ...
                            while($record=mysql_fetch_array($result3)) {
                    	              $t_id=$record[id];
                       	              $t_name=$record[name];
				                      echo "<option value=\"$t_id&id=$id\">$t_name</option>
									  <option value=\"$t_id&id=$id\">----------------------------</option>";
							}
			}
            echo "<option value=\"?am=$a_id&po=$po&tb=$n_id_td&id=$id\">$n_id_tb_name</option>"; 
            echo "<option value=\"?am=$a_id&po=$po&tb=$n_id_td&id=$id\">-----------------------------</option>"; 
